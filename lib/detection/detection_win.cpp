...
typedef HKEY (WINAPI *_SetupDiOpenDevRegKey) (HDEVINFO DeviceInfoSet,PSP_DEVINFO_DATA DeviceInfoData,DWORD Scope,DWORD HwProfile,DWORD KeyType,REGSAM samDesired);
...
string DeviceDetection::Impl::findPortName(HDEVINFO hDevInfo, SP_DEVINFO_DATA* pspDevInfoData) {
	...
	if( (RegQueryValueEx(hDeviceRegistryKey, "PortName", NULL, &dwType, (LPBYTE) portName, &dwSize) == ERROR_SUCCESS) && (dwType == REG_SZ)) {
...
void  DeviceDetection::Impl::loadFunctions() {
	...
	DllSetupDiDestroyDeviceInfoList = (_SetupDiDestroyDeviceInfoList) GetProcAddress(m_hInstLib, "SetupDiDestroyDeviceInfoList");
	DllSetupDiGetDeviceInstanceId = (_SetupDiGetDeviceInstanceId) GetProcAddress(m_hInstLib, "SetupDiGetDeviceInstanceIdA");
	DllSetupDiGetDeviceRegistryProperty = (_SetupDiGetDeviceRegistryProperty) GetProcAddress(m_hInstLib, "SetupDiGetDeviceRegistryPropertyA");
	DllSetupDiOpenDevRegKey = (_SetupDiOpenDevRegKey) GetProcAddress(m_hInstLib, "SetupDiOpenDevRegKey");

	if (!(
		DllSetupDiEnumDeviceInfo != nullptr &&
		DllSetupDiGetClassDevs != nullptr &&
		DllSetupDiDestroyDeviceInfoList != nullptr &&
		DllSetupDiGetDeviceInstanceId != nullptr &&
		DllSetupDiGetDeviceRegistryProperty != nullptr
	)) {
		throw runtime_error(err);
	}
}
