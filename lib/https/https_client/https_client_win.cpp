...
void HttpsClient::doRequest(RequestOptions options) {
  try {
    ...
    hSession = WinHttpOpen(swUAStr, WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, WINHTTP_NO_PROXY_NAME,
                           WINHTTP_NO_PROXY_BYPASS, 0);

    if (!hSession) { handleSystemErrors(); }

    DWORD timeout = (DWORD) options.timeout;


    bResults = WinHttpSetOption (hSession,
                               WINHTTP_OPTION_CONNECT_TIMEOUT,
                               &timeout,
                               sizeof(DWORD)); 
    if (!bResults) { handleSystemErrors(); }
    ...
    if (!hConnect) { handleSystemErrors(); }
    ...
    hRequest = WinHttpOpenRequest(hConnect, method.c_str(), wsPath.c_str(), L"HTTP/1.1", WINHTTP_NO_REFERER, acceptTypes, WINHTTP_FLAG_REFRESH | WINHTTP_FLAG_SECURE);
    if (!hRequest) { handleSystemErrors(); }
    ...
    bResults = WinHttpAddRequestHeaders(hRequest, contentLengthHeader.c_str(), (ULONG)-1L,
