...
begin
  ...
  EasyPayBackend:= TEasyPayBackend.Create(EASYPAY_BACKEND_DEVELOPMENT, 'your-merchant-key');
  ...
  repeat
     ...
     try
      if Input = 'start' then
        ...
        EasyPayBackend.MakePayment('YourTransactionId',
          DeviceHandler.token,
          AmountInCoins,
          AutoDevice.DeviceId,
          'YourCashierIdOrEmptyString')
      else if Input = 'makerefund' then
         EasyPayBackend.MakeRefund(DeviceHandler.easyPayTransactionId, AmountInCoins, AutoDevice.DeviceId)
